...
```typescript
async function listServices(): Promise<APIResponse<ServiceConfig[]>> {
...
```typescript
async function startService(serviceId: string): Promise<APIResponse<{ message: string; service_id: string; status: string }>> {
  const response = await fetch(`/api/v1/services/${serviceId}/start`, {
    method: 'POST',
    headers
  });
  return response.json();
...
```typescript
async function stopService(serviceId: string): Promise<APIResponse<{ message: string; service_id: string; status: string }>> {
  const response = await fetch(`/api/v1/services/${serviceId}/stop`, {
    method: 'POST',
    headers
  });
  return response.json();
...
```typescript
async function restartService(serviceId: string): Promise<APIResponse<{ message: string; service_id: string }>> {
  const response = await fetch(`/api/v1/services/${serviceId}/restart`, {
    method: 'POST',
    headers
  });
  return response.json();
...
```typescript
async function getServiceStatus(serviceId: string): Promise<APIResponse<{
  service_id: string;
  service_name: string;
  status: string;
  enabled: boolean;
  ...
  const response = await fetch(`/api/v1/services/${serviceId}/status`, { headers });
  return response.json();
}
```
...
const PromptShieldDashboard: React.FC = () => {
  ...
  const loadServices = async () => {
    try {
      const response = await fetch('http://localhost:9090/api/v1/services', { headers });
      const data = await response.json();
      if (data.success) {
  ...
  const controlService = async (serviceId: string, action: 'start' | 'stop' | 'restart') => {
    try {
      const response = await fetch(`http://localhost:9090/api/v1/services/${serviceId}/${action}`, {
        method: 'POST',
        headers
      });
      const data = await response.json();
      if (data.success) {
        ...
        console.log(`Service ${action} initiated:`, data.message);
      }
    } catch (error) {
      ...
      <section>
        ...
        {services.map(service => (
          <div key={service.id} className="service-item">
            ...
            <p>Status: <span className={`status ${service.status}`}>{service.status.toUpperCase()}</span></p>
            <p>Last Started: {service.last_started ? new Date(service.last_started).toLocaleString() : 'Never'}</p>
            ...
            {service.error_message && (
              <p className="error-message">Error: {service.error_message}</p>
            )}
          </div>
        ))}
      </section>
