...
export class PromptShieldService {
  ...
  async scanContent(content: string, options?: { aggregate?: boolean }) {
    // Submit as JSON array to get aggregate decisions
    const payload = Array.isArray(content) ? content : [content];
    const response = await this.api.post('/check', payload, {
      headers: { 'Content-Type': 'application/json' }
    });
    return response.data; // { decisions: [...], summary: { total, violations } }
  }
