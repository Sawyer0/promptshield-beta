...
type ViolationHandler interface {
	// Handle handles a detected violation
	Handle(ctx context.Context, violation *types.PolicyViolation) error
	
	// HandleBatch handles multiple violations
	HandleBatch(ctx context.Context, violations []*types.PolicyViolation) error
	
	// GetAction determines the action for a violation
	GetAction(ctx context.Context, violation *types.PolicyViolation) (types.ViolationAction, error)
	
	// ApplyAction applies the determined action
	ApplyAction(ctx context.Context, violation *types.PolicyViolation, action types.ViolationAction) error
	
	// Quarantine quarantines content that violates policies
	Quarantine(ctx context.Context, content []byte, violation *types.PolicyViolation) error
	
	// Redact redacts violating content
	Redact(ctx context.Context, content []byte, violations []*types.PolicyViolation) ([]byte, error)
	
	// Block blocks content that violates policies
	Block(ctx context.Context, violation *types.PolicyViolation) error
}
