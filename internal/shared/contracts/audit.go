...
type AuditLogger interface {
	// Log logs a single audit event
	Log(ctx context.Context, event *types.AuditEvent) error
	
	// LogBatch logs multiple audit events
	LogBatch(ctx context.Context, events []*types.AuditEvent) error
	
	// LogSecurityEvent logs a security-specific audit event
	LogSecurityEvent(ctx context.Context, event *types.SecurityEvent) error
	
	// Flush flushes pending audit logs
	...
	GetConfig() *types.AuditConfig
	
	// SetConfig updates the audit configuration
	SetConfig(config *types.AuditConfig) error
}
