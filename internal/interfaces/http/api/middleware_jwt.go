...
func jwtAuthMiddleware(next http.Handler) http.Handler {
    ...
    if devUserID == "" { devUserID = "dev-user" }
    var devUserName = strings.TrimSpace(os.Getenv("PS_DEV_USER_NAME"))
    if devUserName == "" { devUserName = "Dev User" }
    ...
    return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        ...
        if strings.HasPrefix(r.URL.Path, "/admin/") || strings.HasPrefix(r.URL.Path, "/debug/") {
            next.ServeHTTP(w, r)
            return
        }
...
func splitJWT(token string) (jwtHeader, jwtPayload, []byte, error) {
    ...
    if err != nil { return h, p, nil, err }
    pb, err := b64url(parts[1])
    if err != nil { return h, p, nil, err }
    sb, err := b64url(parts[2])
    if err != nil { return h, p, nil, err }
    if err := json.Unmarshal(hb, &h); err != nil { return h, p, nil, err }
    if err := json.Unmarshal(pb, &p); err != nil { return h, p, nil, err }
    h.raw = parts[0]
    p.raw = parts[1]
    return h, p, sb, nil
}
...
func audMatches(aud interface{}, target string) bool {
    ...
    case []interface{}:
        for _, e := range v {
            if s, ok := e.(string); ok && s == target { return true }
        }
    default:
