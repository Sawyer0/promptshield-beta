...
func newMuxWithMemoryRepo(_ *testing.T) http.Handler {
	pdpClient = nil
	pdpOnce = sync.Once{}
...
func TestRulepackEndpoints_PDP_DenyMatrix(t *testing.T) {
	...
	if recCreate.Code != http.StatusCreated {
		t.Fatalf("create rulepack expected 201, got %d body=%s", recCreate.Code, recCreate.Body.String())
	}
	var cr struct {
		ID string `json:"id"`
	}
	...
	if recReload.Code != http.StatusNoContent {
		t.Fatalf("pdp reload expected 204, got %d", recReload.Code)
	}
